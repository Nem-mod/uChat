...
void mx_init_signup(GtkBuilder *builder, t_uchat_application* app) {
    ...
    *new_scene = SIGNIN;
    ...
    g_signal_connect(app->scenes->signup_scene->bl_signin, "clicked", G_CALLBACK (mx_button_change_scenes), app->scenes->signup_scene->cbdata);
...
void mx_init_signin(GtkBuilder *builder, t_uchat_application* app) {
    t_SCENE *new_scene = malloc(sizeof(t_SCENE));
    ...
    *new_scene = SIGNUP;
    ...
    g_signal_connect(app->scenes->signin_scene->bl_signup, "clicked", G_CALLBACK (mx_button_change_scenes), app->scenes->signin_scene->cbdata);
